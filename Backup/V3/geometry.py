...
class Point:
    ...
    def true_rotate(self, line, angle):
        '''Just like rotate but returns None. Changes self.'''
        velf = self.vector()
        closest_point_on_the_line = (line._direction*velf)*line._direction+line._start
        perp = velf-closest_point_on_the_line
        new_perp = math.cos(angle)*perp
        tang = perp%line._direction
        new_tang = math.sin(angle)*tang
        self._coords = (closest_point_on_the_line.point()+new_perp+new_tang)._coords
...
class Plane:
    ...
    def is_visible(self, point): # important func! executed 1000 times+ per frame
    ...
    def get_color(self, light_dir):
        ...
        r =  tuple([int(round(e*255)) for e in colorsys.hsv_to_rgb(self.color[0],self.color[1],self.color[2]*abs(self._direction*light_dir))])
        return r
